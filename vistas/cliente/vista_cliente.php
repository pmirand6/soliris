...
<html>
    ...
    <body>
        <table cellpadding="0" cellspacing="0" border="0" class="table table-bordered" id="TBLClientes" style="font-size: 13px">
            <thead>
                <tr>
                    <th> id </th>
                    <th> Código </th>
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $arr_tbody .= '<td><b class="pointer" onclick="callCliente(\'' . $row["id"] . '\')">' . ucwords(strtolower($row["codigo"])) . '</b></td>';
    $arr_tbody .= '<td>' . $row["nombre"] . '</td>';
    $arr_tbody .= '<td>' . $row["ciudad"] . '</td>';
    $arr_tbody .= '<td>' . $row["direccion"] . '</td>';
    $arr_tbody .= '<td>' . $row["convenio"] . '</td>';
    $arr_tbody .= "<td><div class=\"TBL TBL-" . str_replace(" ", "_", $row["estado"]) . "\" title=\"" . $row["estado"] . "\"><p class=\"hidden\">" . $row["estado"] . "</p></div></td>";
    if ($row["estado"] == 'Activo') {
        $arr_tbody .= '<td><button type=\'button\' class=\'del_can btn btn-danger\' title=\'Desactivar\' style=\'margin-left: 13px;\' onclick="delCliente(\'' . $row["id"] . '\')"><i class=\'fa fa-ban\'></i></button></td>';
    } else {
        $arr_tbody .= '<td><button type=\'button\' class=\'act_can btn btn-success\' title=\'Activar\' style=\'margin-left: 13px;\' onclick="ActivarCliente(\'' . $row["id"] . '\')"><i class=\'fa fa-undo\'></i></button></td>';
    }
    $arr_tbody .= '</tr>';
};
