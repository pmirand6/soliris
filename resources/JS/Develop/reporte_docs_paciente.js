  ...
  $(document).ready(function () {
    ...
    $('#DataRVP').dataTable({
      ...
      aoColumns: [
        ...
        { mData: 'vacunacion', bSearchable: true, sWidth: '10%', sClass: 'center' },
...
function reloadTBLinitial(init, fin) {
  ...
  if (init != '' && fin != '') {
    $.get(`${aplicacion}/ajax/ajx.reporte_docs_paciente.php?ini=${init}&fin=${fin}`, function (data) {
      doRefresh(data, '#DataRVP')
    })
  } else {
    $.get(`${aplicacion}/ajax/ajx.reporte_docs_paciente.php`, function (data) {
      doRefresh(data, '#DataRVP')
    })
  }
}
