    ...
    $(document).ready(function() {
        ...
        $("#TBLAPM").dataTable( {
            ...
            { "data": function (data, type, dataToSet) {
                    return "<b class='pointer' onclick='callAPM("+data.id+")' title='Editar "+data.nombre+"'>"+data.nombre+"</b>";
            }},
            { "data": "estado", "bSearchable": true, "sWidth": "3%","sClass": "center"},
            { "data": function (data, type, dataToSet){
                if (data.estado == 'Inactivo') {
                    //return "<span class='pointer TBL TBL-Reasignado-Drogueria' title='Activar APM' onclick='reactAPM("+data.id+");'></span>";
                    return "<button type=\'button\' class=\'del_can btn btn-success text-center\' title=\'Activar APM\' style=\'margin-left: 13px;\' onclick='reactAPM("+data.id+");'><i class=\'fa fa-undo\'></i></button>";

                } else {
                    //return "<span class='pointer TBL TBL-Cancela-Paciente' title='Desactivar APM' onclick='delAPM("+data.id+");'></span>";
                    return "<button type=\'button\' class=\'del_can btn btn-danger\' title=\'Desactivar APM\' style=\'margin-left: 13px;\' onclick='delAPM("+data.id+");'><i class=\'fa fa-ban\'></i></button>";
                }
