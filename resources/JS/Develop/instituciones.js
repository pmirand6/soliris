...
var aplicacion = getCurrentHostname() + '/' + getUrlHTTP();
 $(document).ready(function() {
    ...
    $("#DataInstitucion").dataTable( {
        ...
        { "data": function (data, type, dataToSet){ // 1
            return "<b class='pointer' onclick='callInst("+data.id+");'>"+data.nombre+"</b>"
        }, "sWidth": "50%", "sClass": "center"},
        ...
        { "data": function (data, type, dataToSet){
            //return "<span title='Eliminar' onclick='delInst("+data.id+", \""+data.nombre+"\");'><i class=\"fa fa-trash-o\" aria-hidden=\"true\"></i></span>";
            return "<button type='button' class='del_inst btn btn-danger' title='Eliminar' style='margin-left: 13px;' onclick='delInst("+data.id+", \""+data.nombre+"\");'><i class='fa fa-trash-o'></i></button>"
        }}, // 9
        ],
        "columnDefs":[
                    {
...
function callInst(id){
    $('#win').window('open');  // close a window
    $('#win_inst').attr('src', aplicacion + '/administrador/institucion.php?id=' + id);
}
