            ...
            <table class="table table-bordered table-responsive" id="DataTRPA">
                <thead>
                    <tr>
                        ...
                        <?php
						echo "<tr><td>APM</td>";
							$SQL = "SELECT fecha FROM v_meses_apm ";
...
require_once '../config/config.php';
                    ...
                    $SQL = "SELECT id, nombre FROM apm ORDER BY nombre asc;";
                    $response = mysqli_query($db, $SQL);
                    if ($response){
...
require_once '../config/config.php';
    ...
    $SQLRI = "SELECT count(id) as CANT FROM soliris_maestro WHERE apm='$apm' AND concat(month(fecha_venta),'/',year(fecha_venta))='$fecha' AND estado='NP' AND tipo='$tipo';";
