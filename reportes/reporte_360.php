			...
			<table class="table table-bordered" id="tblPrincipal">
				...
				<tbody>
					...
					if (strcasecmp($campo, '0') == 0) {
						...
						$where_p = "FROM maestro_ventas as m
				LEFT JOIN patologia pat ON pat.id = m.patologia_id
				LEFT JOIN
				paciente as p
				ON
				p.id = m.paciente_id
				...
				order by p.nombre_completo asc";
					...
					$query_pac_v = "SELECT
p.id,
p.nombre_completo AS pac_nom,
p.sexo AS pac_sexo,
p.fecha_nac AS pac_fn,
pat.patologia_nombre AS pac_pat,
(SELECT FU_GET_FECHA_RECETA(p.id)) AS pac_fc,
...
SUM(m.unidades) AS suma_unidades
			$where_p";
							...
							while ($row_pac_v = mysqli_fetch_assoc($result_pac_v)) {
								...
								if (!empty($campo)) {
									...
									if (strcasecmp($campo, '0') == 0) {
										...
										$where = "FROM maestro_ventas AS m
										LEFT JOIN paciente AS p
										  ON p.id = m.paciente_id
										LEFT JOIN medico AS me
										  ON m.medico_id = me.id
										LEFT JOIN presentacion pre ON pre.id = m.id
										LEFT JOIN canales c ON c.id = m.canal_id
										LEFT JOIN institucion i ON i.id = m.institucion_id
										LEFT JOIN apm a ON a.id = me.apm_id
										LEFT JOIN maestro_estado mae ON mae.id = m.estado_id
										LEFT JOIN maestro_ventas_tipo mvt ON mvt.id
										WHERE m.estado_id IN (23)
										AND m.paciente_id = $id_pac $v_plus
										order by p.id asc";
									} else {
										$inicial_split = str_replace("_", " ", $_GET["inicial"]);
										if (strcasecmp($campo, 'p.edad') == 0) {
								...
								p.id asc";
									...
									$sel = "SELECT
									c.canal, /*0*/
									i.nombre AS institucion, /*1*/
									m.fecha_venta, /*2*/
									me.id AS id_med, /*3*/
									UPPER(CONCAT(me.apellido, ', ', me.nombre)) AS nombre_med, /*4*/
									pre.valor AS presentacion, /*5*/
									m.unidades, /*6*/
									m.edad, /*7*/
									a.nombre_completo AS apm, /*8*/
									mvt.valor AS tipo, /*9*/
									p.nombre_completo AS nombre, /*10*/
									mae.valor AS estado, /*11*/
									m.id,/*12*/
									me.c_atencion /*13*/
						$where";
					...
					?>
										...
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.edad=|<?= $m[7]; ?>|;<?= $_GET["plus"]; ?>"><?= $m[7]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20p.Sexo=|<?= $row_pac_v["pac_sexo"]; ?>|;<?= $_GET["plus"]; ?>"><?= $row_pac_v["pac_sexo"]; ?></a>
										</td>
										<td class="lef">
											<?= $day; ?>
											/<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&lim=3&plus=%20and%20month(m.fecha_venta)=|<?= $month; ?>|;<?= $_GET["plus"]; ?>>" title="Filtro por Mes"><?= $month; ?></a>
											/<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&lim=3&plus=%20and%20year(m.fecha_venta)=|<?= $year; ?>|;<?= $_GET["plus"]; ?>>" title="Filtro por Año"><?= $year; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20p.patologia=|<?= $row_pac_v["pac_pat"]; ?>|;<?= $_GET["plus"]; ?>>"><?= $row_pac_v["pac_pat"]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.canal=|<?= $m[0]; ?>|;<?= $_GET["plus"]; ?>>"><?= $m[0]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.institucion=|<?= $m[1]; ?>|;<?= $_GET["plus"]; ?>>"><?= $m[1]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.id_medico=|<?= $m[3]; ?>|;<?= $_GET["plus"]; ?>"><?= $m[4]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.dosis=|<?= $m[5]; ?>|;<?= $_GET["plus"]; ?>"><?= $m[5]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.unidades=|<?= $m[6]; ?>|;<?= $_GET["plus"]; ?>"><?= $m[6]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.apm=|<?= $m[8]; ?>|;<?= $_GET["plus"]; ?>"><?= $m[8]; ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=maestro_ventas&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20m.tipo=|<?= $m[9]; ?>|;<?= $_GET["plus"]; ?>"><?= fl_tipo_rep($m[9]); ?></a>
										</td>
										<td class="lef">
											<a href="reporte_360.php?tabla=medico&campo=<?= $campo; ?>&inicial=<?= $inicial; ?>&lim=3&plus=%20and%20me.c_atencion=|<?= $m[13]; ?>|;<?= $_GET["plus"]; ?>"><?= $m[13]; ?></a>
										</td>
					<?php
										switch ($m[6]) {
