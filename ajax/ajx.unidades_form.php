...
if(isset($_POST["oper"]) AND $_POST["oper"] == "Guardar"){
    ...
    if (isset($id) AND $id != ""){
            $SQL = "SELECT FU_UP_UNIDADES('$nombre', '$familia', '$usuario', '$id') as response";
        } else {
            /* Verifico que no exista el Medico en la base */
                $arr_exists = mysqli_query($db, "SELECT id FROM auxiliar WHERE valor = '$nombre' AND tipo = 'unidades';");
            /* Determinar el número de filas del resultado */
                $row_cnt = mysqli_num_rows($arr_exists);
                if ($row_cnt == 0){
                    $SQL = "SELECT FU_NEW_UNIDADES('$nombre', '$familia', '$usuario') as response";
                }else{
...
//         echo $SQL;
            $response = MySQL_sendFunctionAudit("$SQL", "unidades_form.php", "1");
            echo("$response");
    }
