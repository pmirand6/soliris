...
BEGIN
  IF old.id <> new.id THEN
    ...
    SET usuario = USER(),
        ...
        valor_nuevo = NEW.id;
  END IF;
  IF old.codigo_apm_np <> new.codigo_apm_np THEN
    INSERT INTO soliris_log
    SET usuario = USER(),
        fecha = NOW(),
        accion = 'update',
        registro = 'codigo_apm_np',
        tabla = 'apm',
        id_registro = old.id,
        valor_anterior = OLD.codigo_apm_np,
        valor_nuevo = NEW.codigo_apm_np;
  END IF;
