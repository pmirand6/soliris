...
BEGIN
  ...
  -- Actualizo el estado del paciente a "Pendiente"

  SET @query_pend_pac = CONCAT("UPDATE paciente set estado_id = 7 where id = ?");
  PREPARE stmt FROM @query_pend_pac;
  EXECUTE stmt USING @p_id_paciente;
