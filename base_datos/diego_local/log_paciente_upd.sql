...
-- Volcando estructura para disparador soliris.log_paciente_upd
SET @OLDTMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';
DELIMITER //
CREATE TRIGGER `log_paciente_upd` BEFORE UPDATE ON `paciente` FOR EACH ROW BEGIN


/*Control por cambio de subestado*/

if (NEW.sub_estado_id=9 OR NEW.sub_estado_id=10) then

	SET NEW.estado_id=12;

END if;

if old.id<>new.id THEN
	INSERT INTO soliris_log SET 
		usuario= USER(), 
		fecha= NOW(), 
		accion='update', 
		registro='id', 
	...
	if old.apellido<>new.apellido THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.apellido, 
			valor_nuevo=NEW.apellido; 
	END If;
	...
	if old.nombre_completo<>new.nombre_completo THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.nombre_completo, 
			valor_nuevo=NEW.nombre_completo; 
	END If;
	if old.fecha_nac<>new.fecha_nac THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.fecha_nac, 
			valor_nuevo=NEW.fecha_nac; 
	END If;
	if old.sexo<>new.sexo THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.sexo, 
			valor_nuevo=NEW.sexo; 
	END If;
	...
	if old.ciudad<>new.ciudad THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.ciudad, 
			valor_nuevo=NEW.ciudad; 
	END If;
	if old.pais_id<>new.pais_id THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.pais_id, 
			valor_nuevo=NEW.pais_id; 
	END If;
	...
	if old.os_id<>new.os_id THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.os_id, 
			valor_nuevo=NEW.os_id; 
	END If;
	...
	if old.c_gestar<>new.c_gestar THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.c_gestar, 
			valor_nuevo=NEW.c_gestar; 
	END If;
	...
	if old.sub_estado_id<>new.sub_estado_id THEN
		INSERT INTO soliris_log SET 
			...
			fecha= NOW(), 
			accion='update', 
			registro='sub_estado_id', 
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.sub_estado_id, 
			valor_nuevo=NEW.sub_estado_id; 
	END If;
	...
	if old.fecha_modificacion<>new.fecha_modificacion THEN
		INSERT INTO soliris_log SET 
			...
			tabla='paciente', 
			id_registro=old.id, 
			valor_anterior=OLD.fecha_modificacion, 
			valor_nuevo=NEW.fecha_modificacion; 
	END If;
